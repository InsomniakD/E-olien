...
static void humidite_callback_exti(uint16_t pin);
...
uint8_t humidite_valeur(void)
{
	...
	humidite_init(GPIOB, GPIO_PIN_7);
	while(1)
	{
...
static void humidite_process_ms(void)
{
	if(t)
		t--;
}
	...
	switch(state)
	{
		case INIT:
			if(initialized)
			{
				...
				Systick_add_callback_function(&humidite_process_ms);
				EXTIT_set_callback(&humidite_callback_exti, EXTI_gpiopin_to_pin_number(DHT11_pin), FALSE);
			}
			else
			{
